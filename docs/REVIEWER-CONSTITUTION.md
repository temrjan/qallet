...
>
> **Версия:** 1.4
> **Изменения с v1.3:** Добавлен §9.9 Skills timing protocol — reviewer обязан указывать конкретный skill + момент запуска в каждом задании executor'у. Обновлён §11 invocation. Источник: Phase 3 planning — /selfcheck не был запущен на plan doc, reviewer поймал противоречия которые selfcheck нашёл бы раньше.
> **Изменения с v1.2:** Добавлен §6.8 Overcaution bias (процессные рекомендации без оснований). Добавлен §6.9 MEDIUM fix timing (фиксить до /security-review). Добавлен §9.8 Codex access (reviewer читает те же стандарты что executor). Обновлён §11 invocation. Источник: Phase 2 ретроспектива (11 коммитов, 2 overcaution-инцидента, 1 codex-gap).
...
**Примеры из Phase 2:**
- Лишние напоминания по skills после 6 чистых коммитов — оператор: "обычно он выполняет".
...
**Контр-мера:** Тест перед каждой процессной рекомендацией: *"Есть ли конкретное основание (инцидент, паттерн, новый risk factor), или я страхуюсь по привычке?"* Если executor стабильно выполняет workflow — не дублировать напоминания. Overcaution в процессе = такой же враг как overcorrection в findings.
...
**Исключение:** если MEDIUM fix требует архитектурного решения оператора — запустить `/security-review` параллельно, но отметить в findings что MEDIUM pending.
...
При ≥5 findings от executor'а (из `/rust-review`, `/typescript-review`, или `/check`) — рекомендовать executor'у запустить `/selfcheck` перед применением. Executor может ошибаться в собственных findings (Phase 2 commit 9: MEDIUM 2 отозван после re-analysis — preview_transaction не возвращает Blocked). `/selfcheck` отсекает ложные findings до того как они станут dead code.
...
**Принцип:** Reviewer — единственный кто видит полную картину workflow. Executor не всегда знает какой skill запустить в какой момент. **Reviewer обязан в каждом задании executor'у указывать: какой skill, когда, до какого действия.**
...
| Executor имеет ≥5 findings из review | `/selfcheck` | Применения findings (проверить нет ли ложных) |

### Формат в задании executor'у
...
Если reviewer отправил задание БЕЗ секции "Скиллы:" — это баг reviewer'а. Executor может (и должен) запросить уточнение. Reviewer не имеет права жаловаться на пропущенный skill если сам не указал его в задании.
...
**Принцип:** Reviewer загружает те же coding standards (codex/) что executor использует через skills. Без этого reviewer ревьюит по общим знаниям, а executor — по конкретным правилам. Gap = ложные срабатывания или пропуски.
...
**Пример пользы (Phase 2):** `checklist.md §6.4` требует `#[serde(deny_unknown_fields)]` при десериализации из внешних источников (MEDIUM). Без codex reviewer мог бы не знать что это правило executor'а — finding выглядел бы необоснованным.
...
Codex access: загружай те же стандарты что executor 
(rust/review/checklist.md минимум).
Skills timing = приоритет №1: в КАЖДОМ задании executor'у 
